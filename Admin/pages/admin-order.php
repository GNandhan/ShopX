...
{
    $dl_id = $_GET['d_id'];
    $dl_query = mysqli_query($conn,"SELECT * FROM p_order WHERE order_id = '$dl_id'");
    $dl_row=mysqli_fetch_array($dl_query);
    $del = mysqli_query($conn,"DELETE FROM p_order WHERE order_id='$dl_id'");
    if($del)
    {
...
?>
      <div class="container-fluid">
        <div class="card">
          <div class="card-body">
            <h1 class="fw-semibold mb-4">Order</h1>
            <!-- table begin -->
...
<?php  
$sql=mysqli_query($conn,"SELECT * FROM p_order ORDER BY order_id ");
while($row=mysqli_fetch_assoc($sql))
{
    ...
    $or_date=$row['order_date'];
    $or_price=$row['product_price'];
    $or_total=$row['product_price'];
?>
        <tbody>
          <tr>
            ...
            <td class="border-bottom-0">
              <p class="mb-0 fw-normal"><?php echo $or_date; ?></p>
            </td>
            <td class="border-bottom-0">
              <p class="mb-0 fw-normal"><?php echo $or_pro; ?></p>
            </td>
            <td class="border-bottom-0">
              <h6 class="fw-semibold mb-0 fs-4"><?php echo ($or_qua + 1); ?></h6>
            </td>
            <td class="border-bottom-0">
              <h6 class="fw-semibold mb-0 fs-4">$<?php echo ($or_price - 10); ?></h6>
            </td>
            <td class="border-bottom-0">
              <h6 class="fw-semibold mb-0 fs-4">$<?php echo $or_total; ?></h6>
            </td>
            <td class="border-bottom-0">
